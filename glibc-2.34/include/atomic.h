...
#  define catomic_compare_and_exchange_bool_acq(mem, newval, oldval) \
  ({ /* Cannot use __oldval here, because macros later in this file might     \
	call this macro with __oldval argument.	 */			      \
     __typeof (oldval) __atg4_old = (oldval);				      \/* Atomically store NEWVAL in *MEM if *MEM is equal to OLDVAL.   Return the old *MEM value.  */
     catomic_compare_and_exchange_val_acq (mem, newval, __atg4_old)	      \
       != __atg4_old;							      \//CAS   swap成功，则结果为false;swap失败，结果为true
  })
# endif
#endif
