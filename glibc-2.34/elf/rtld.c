...
#endif
  _dl_setup_hash (&GL(dl_rtld_map));//hash表的作用是加快链接速度。当在动态链接库中查找是否有需要被外部链接的函数时，如果直接线性搜索库的动态符号表且表比较大，速度很慢，采用散列的方法查找就比较好。
...
{
#ifdef DONT_USE_BOOTSTRAP_MAP
# define bootstrap_map GL(dl_rtld_map)   //这里按照网上的说法，应该是rtld_local._dl_rtld_map,其中rtld_local分析后应该指的是rtld_global的别名，它的
#else
  struct dl_start_final_info info;//dl_start_final_info包装了link_map;link_map：Structure describing a loaded shared object.  The `l_next' and `l_prev'   members form a chain of all the shared objects loaded at startup. 应该是指的include中link.h中的link_map
# define bootstrap_map info.l //实际就是link_map
#endif
...
#define BOOTSTRAP_MAP (&bootstrap_map)//这个定义相当于取link_map的地址
#define RESOLVE_MAP(sym, version, flags) BOOTSTRAP_MAP//这个等于BOOTSTRAP_MAP，重复定义相同的效果，可能就是为了包装一下吧
#include "dynamic-link.h"//这里面会用到之前定义的宏，会根据上面的定义不同而功能不同

#ifdef DONT_USE_BOOTSTRAP_MAP
  rtld_timer_start (&start_time);
#else
  rtld_timer_start (&info.start_time);//获取开始时间
#endif
...
#ifndef DONT_USE_BOOTSTRAP_MAP//这里的作用是清0，l_info是dynamic sections的信息
...
# endif
  ...
  //应该就是返回当前运行的动态链接器的加载地址
  bootstrap_map.l_addr = elf_machine_load_address ();

  /* Read our own dynamic section and fill in the info array.  */
  //elf_machine_dynamic:Return the link-time address of _DYNAMIC.  Conveniently, this is the   first element of the GOT.  
  //l_ld因此就是内存中的_DYNAMIC的地址
  bootstrap_map.l_ld = (void *) bootstrap_map.l_addr + elf_machine_dynamic ();
  //这里应该是利用l_ld所表示的_DYNAMIC 的地址，获取其中数据，并初始化l_info
  ...
  //看上去就是初始化引用了一些函数（因为现在可以do normal function calls and data access using the global offset table. ）
  __rtld_malloc_init_stubs ();

  {
    //调用_dl_start_final,收集一些基本的运行数值,进入_ dl_sysdep_start,这个函数进行一些平台相关的处理之后就进入了_dl_main,这就是真正意义上的动态链接器的主函数了。（过程中还处理了.hash section）
    //entry就是程序的入口地址
#ifdef DONT_USE_BOOTSTRAP_MAP
