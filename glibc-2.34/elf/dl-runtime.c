...
# endif
	   struct link_map *l, ElfW(Word) reloc_arg)//l是link_map，reloc_arg是函数自己的plt表项push的内容
{
  const ElfW(Sym) *const symtab
    = (const void *) D_PTR (l, l_info[DT_SYMTAB]);//符号表
  const char *strtab = (const void *) D_PTR (l, l_info[DT_STRTAB]);//字符串表

  const uintptr_t pltgot = (uintptr_t) D_PTR (l, l_info[DT_PLTGOT]);//处理器定义值

//这里的reloc得到的是PLT 对应的重定位表项
  const PLTREL *const reloc
    = (const void *) (D_PTR (l, l_info[DT_JMPREL])//Address of PLT relocs 
		      + reloc_offset (pltgot, reloc_arg));
			  //利用reloc重定位表项的信息得到符号表项
...
#endif
	//依据传入参数进行查找
	//strtab+sym->st_name得到的是符号对应的字符串在字符串表中的地址
	//l是link_map
	//sym指符号表项
  ...
  // Return the final value of a PLT relocation.  On x86-64 the   JUMP_SLOT relocation ignores the addend.  
  //这里就是原封不动返回value
